...
export default function PracticeScenarioPage() {
  ...
  // Fetch existing user progress for this scenario
  useEffect(() => {
    const fetchUserProgress = async () => {
      ...
      try {
        ...
        if (data.success) {
          const scenarioProgress = data.data.find(item => item.id === parseInt(id));
          if (scenarioProgress) {
            setUserAnswers(scenarioProgress.answers || {});
            setScore(scenarioProgress.score || 0);
            setCompleted(scenarioProgress.is_completed || false);
            if (scenarioProgress.is_completed) {
              setShowResults(true);
            }
          }
        }
  ...
  const finishScenario = async () => {
    ...
    await saveProgress(calculatedScore);
  };
  ...
  const saveProgress = async (finalScore) => {
    ...
    try {
      const timeTaken = (currentScenario.time_estimate || currentScenario.timeEstimate || "15 min");
      const minutes = parseInt(timeTaken.split(' ')[0]) || 15;
      const timeUsed = (minutes * 60) - timer;
