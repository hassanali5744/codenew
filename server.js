...
app.put("/api/tools/:id", async (req, res) => {
  try {
    ...
    const [result] = await pool.query(`UPDATE tools SET ${fields} WHERE id = ?`, values);
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO practice_questions (
        scenario_id, question_text, question_type, difficulty, points,
        time_limit, options, correct_answer, explanation,
        code_template, code_snippet, scenario_context,
        expected_output, order_index, is_active
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        scenario_id,
        question_text,
...
app.delete('/api/practice/scenarios/:id', async (req, res) => {
  const { id } = req.params;
  try {
